...
async function main() {
  ...
  try {
    ...
    const [duplicates] = await connection.query(`
      SELECT duid, COUNT(*) as duplicate_count, GROUP_CONCAT(id) as site_ids
      FROM sites
      GROUP BY duid
      HAVING COUNT(*) > 1
    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach(dup => {
        console.log(`    - ${dup.duid || 'null'}: ${dup.duplicate_count} copies (IDs: ${dup.site_ids})`);
      });
    } else {
    ...
    // Step 4: Truncate all tables (in correct order due to foreign keys)
